...
public void run() {
    ...
    switch (num) {
        case 1: {//1-->Printer is requested
            System.out.println("A Printer is requested. by process " + ManagementFactory.getRuntimeMXBean().getName());
            s = rm.printers;
            break;
        }
        case 2: {//2-->Plotter is requested
            System.out.println("A Plotter is requested. by process " + ManagementFactory.getRuntimeMXBean().getName());
            s = rm.plotters;
            break;
        }
        case 3: {//3-->Scanner is requested
            System.out.println("A Scanner is requested. by process " + ManagementFactory.getRuntimeMXBean().getName());
            s = rm.scanners;
        break;
        }
    }
